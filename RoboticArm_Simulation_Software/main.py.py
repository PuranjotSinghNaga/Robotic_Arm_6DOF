...
class JointControlApp:
    ...
    def execute_trajectory(self, target_position, steps=100, time_step=0.01):
        ...
        current_position = [p.getJointState(robot_id, joint_index)[0] for joint_index in range(num_joints)]
        
        # Generate linear trajectory for each joint
        for step in range(steps):
            interpolated_position = [
                np.linspace(current_pos, target_pos, steps)[step]
                for current_pos, target_pos in zip(current_position, target_position)
            ]
            
            # Apply interpolated positions to each joint
            for joint_index in range(num_joints):
                p.setJointMotorControl2(bodyUniqueId=robot_id, jointIndex=joint_index, controlMode=p.POSITION_CONTROL, targetPosition=interpolated_position[joint_index])
